...
import { utcOffsetHoursFor } from './timezoneOffset.mjs';
...
function getUtcOffsetHoursForBirth(person) {
  if (!person.timezone) {
    throw new Error(
      `Missing IANA timezone for ${person.name ?? person.id}. Store birth data as local time + IANA zone.`
    );
  }

  const { year, month, day } = parseBirthDate(person.birth_date);
  const { hour, minute } = parseBirthTime(person.birth_time);
  return utcOffsetHoursFor(person.timezone, year, month, day, hour, minute);
}
